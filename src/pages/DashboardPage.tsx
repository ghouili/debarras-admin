...
const emptyStats = {
  contacts: {
    total: 0,
    byStatus: { nouveau: 0, en_cours: 0, fermee: 0 },
  },
  devis: {
    total: 0,
    byStatus: { nouveau: 0, traite: 0, gagne: 0, perdu: 0 },
  },
}
...
export function DashboardPage() {
  ...
  const displayData = data ?? emptyStats
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {['nouveau', 'en_cours', 'fermee'].map((status) => (
              <div key={status} className="flex items-center gap-2 rounded-[10px] border border-[hsl(var(--border))] px-3 py-2">
                <Badge variant={status === 'fermee' ? 'success' : status === 'en_cours' ? 'warning' : 'info'}>{status}</Badge>
        ...
        <Card>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {['nouveau', 'traite', 'gagne', 'perdu'].map((status) => (
              <div key={status} className="flex items-center gap-2 rounded-[10px] border border-[hsl(var(--border))] px-3 py-2">
                <Badge
                  variant={
                    status === 'gagne'
